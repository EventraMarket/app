...
export default function EconomyMarkets() {
  ...
  const games = (data?.games ?? []).filter((g) => g.category?.toLowerCase() === "economy");
  return (
    <div className="min-h-screen text-[var(--color-foreground)] pt-20 px-4">
      <h1 className="text-2xl font-bold mb-6">Economy Markets</h1>
      {isFetching && (
      ...
      {!isFetching && games.length === 0 && (
        <p className="text-center text-[#D9A650]/60 py-20">No economy markets available yet.</p>
      )}
      {!isFetching && games.length > 0 && (
