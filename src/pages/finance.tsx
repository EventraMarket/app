...
export default function FinanceMarkets() {
  ...
  const games = (data?.games ?? []).filter((g) => g.category?.toLowerCase() === "finance");
  return (
    <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)] pt-20 px-4">
      <h1 className="text-2xl font-bold mb-6">Finance Markets</h1>
      {isFetching && (
      ...
      {!isFetching && games.length === 0 && (
        <p className="text-center text-[#D9A650]/60 py-20">No finance markets available yet.</p>
      )}
      {!isFetching && games.length > 0 && (
